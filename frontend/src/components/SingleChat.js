...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const { selectedChat, setSelectedChat, user, notification, setNotification, setMe, callNotification, setCallNotification } =
    ChatState();
  ...
  useEffect(() => {
    ...
    // === call notification
    socket.on("call alert", (newCallAlert) => {
      console.log("new call newCallAlert ===>", newCallAlert);
      if (
        !selectedChatCompare ||
        selectedChatCompare._id === newCallAlert.chat._id
      ) {
        if (!callNotification.includes(newCallAlert)) {
          setCallNotification([newCallAlert, ...callNotification]);
          setFetchAgain(!fetchAgain);
        }
      } 
    });
